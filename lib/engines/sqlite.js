...
import { ensureDirSync, exists, readTextFile, writeTextFile, quoteIdentifier, quoteLiteral } from '../utils.js';
...
export class SqliteEngine {
  ...
  dumpSchema() {
    ...
    const schema = db
      ...
      .all();
    return schema.map((row) => row.sql).filter(Boolean).join(';\n\n') + ';\n';
  }

  exportSql(outputPath) {
    writeTextFile(outputPath, this.dumpSchema());
    return outputPath;
  }
}
