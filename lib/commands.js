...
async function recoverMysqlAuth(engine, confirm) {
  ...
  const settings = engine.getLoginSettings?.() || {};
  const label = settings.user ? `MySQL user "${settings.user}"` : 'the MySQL user from DATABASE_URL';
  if (!(await confirm(`${label} failed authentication. Create or update that user and grant access to "${settings.database}" using sudo mysql?`))) {
    return false;
  }
  const result = engine.createOrUpdateLoginRole();
...
async function recoverServerConnection(engineName, engine, confirm) {
  ...
  if (!(await confirm('MySQL refused the connection on the configured host/port. Start the local mysql service now?'))) {
    return false;
  }
