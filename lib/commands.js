...
import { formatList, listSqlFiles, normalizeEngineName, resolvePath, timestampTag } from './utils.js';
...
async function ensureClientReady(config, confirm, options = {}) {
  ...
  if (options.nonInteractive) {
    throw new Error(
      `Missing required client: ${toolKey}. Install it from the CLI with \`node db.js install ${toolKey}\`.`
    );
  }
...
async function runWithRecovery(action, context, recoveryAction = null, attemptsRemaining = 3) {
  try {
    ...
    // The web UI runs non-interactively: never trigger sudo/auto-install or
    // schema re-init from a browser click. Surface the error instead.
...
Commands:
  ...
  ui [--port <n>]         Start the local web dashboard (default port 4321)
  help                    Show this help
...
export async function runCommand(context) {
  ...
  switch (command) {
    ...
    case 'ui':
    case 'serve': {
      // Dynamic import avoids a static commands.js <-> server.js import cycle.
      const { startUiServer } = await import('./server.js');
      const port = Number(options.port) || 4321;
      await startUiServer({ cwd, port });
      return;
    }
